...
7. **Update CHANGELOG.md** — after every feature addition, bugfix, or significant change, add an entry under `[Unreleased]` grouped by category. Keep entries concise and user-facing. This is the canonical record of what ships in each release. When adding a new `### Section` heading that is developer‑facing (not useful to end users), add it to `_devOnlySections` in `lib/widgets/whats_new_sheet.dart` so the in‑app What's New sheet hides it.
