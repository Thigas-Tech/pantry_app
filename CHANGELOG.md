...
### Fixed
- **Feedback form fixes**: Fixed dropdown selection not responding to changes
  (switched `initialValue` to `value` on `DropdownButtonFormField`). Added
  support for attaching multiple screenshots to feedback submissions. Fixed
  missing Portuguese (pt) translations for all feedback-related strings.
  Replaced misleading "Retry" label on screenshot removal with proper
  "Remove" string. Added `feedbackEnabled` config guard to hide the feedback
