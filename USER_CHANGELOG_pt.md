...
- Os itens da despensa podem agora usar unidades de miligrama e micrograma,
  para que produtos como vitaminas e suplementos mantenham as suas
  quantidades corretas.

- Se as suas credenciais do Open Food Facts forem rejeitadas, o aplicativo
  agora avisa isso claramente e nao continua a tentar. Use o seu nome de
  utilizador do Open Food Facts (e nao o seu endereco de e-mail) como id de
  utilizador na configuracao do aplicativo.
...
- As fotos de um produto introduzido manualmente podem agora ser geridas a
  partir da pagina do produto: toque numa foto para a ver a ecra inteiro,
  substitua-a ou remova-a. Se remover uma por engano, pode anular a acao, e
  os ficheiros que eliminar sao limpos ao sair da pagina.

- Ao guardar um produto introduzido manualmente, a app verifica primeiro se
  esse codigo de barras ja existe no Open Food Facts. Se existir, o envio e
  interrompido e e informado de que o produto ja esta la, em vez de o
  sobrescrever.
- Se o envio de um produto falhar por um motivo que pode corrigir — por
  exemplo, dados de produto invalidos — a app explica agora o que correu
  mal, em vez de mostrar um erro generico.
- As fotos carregam uma de cada vez e uma foto que falha ja nao bloqueia as
  outras; quando a ligacao voltar, so as fotos em falta sao enviadas de
  novo, nao as que ja chegaram.
- As fotos dos produtos sao comprimidas antes do envio, para chegarem mais
  depressa e gastarem menos dados moveis.
- Ao guardar um produto introduzido manualmente, ve agora o progresso do
  envio para o Open Food Facts: primeiro os detalhes do produto e depois
  cada foto enquanto e carregada.
- O formulario fica aberto enquanto o produto esta a ser enviado e fecha-se
  sozinho com uma confirmacao quando termina com sucesso.
- Se o envio falhar, pode tentar de novo diretamente no formulario, e a
  mensagem indica o motivo — por exemplo, sem ligacao a internet ou Open
  Food Facts ocupado.
...
- Ao registrar um preco, pode indicar o tamanho do pacote (por exemplo,
  ...
  escondido se tiver escolhido ocultar os precos.
- Os totais da despensa e o preco medio dos itens agora levam em conta a
  quantidade de cada produto que tem, para que os numeros reflitam o estoque
  real.
