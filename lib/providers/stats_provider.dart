...
Future<PantryStats> stats(Ref ref) async {
  ...
  final results = await Future.wait([
    ...
    database.rawQuery(
      'SELECT COUNT(*) AS cnt, COALESCE(SUM(cost_at_time), 0) AS total_cost'
      ' FROM recipe_history'
      ' WHERE made_at >= ?',
      [
        DateTime.now()
    ...
    database.rawQuery(
      'SELECT recipe_id, COUNT(*) AS cnt FROM recipe_history'
      ' GROUP BY recipe_id ORDER BY cnt DESC LIMIT 1',
    ),
  ...
  final recipeHistoryRows = results[11] as List<Map<String, dynamic>>;
  final mostCookedRows = results[12] as List<Map<String, dynamic>>;
  ...
  final mealsCooked = (recipeHistoryRows.first['cnt'] as num?)?.toInt() ?? 0;
  final totalRecipeCost =
      (recipeHistoryRows.first['total_cost'] as num?)?.toDouble() ?? 0.0;
  var mostCookedRecipe = '';
  if (mostCookedRows.isNotEmpty) {
    final topRecipeId = (mostCookedRows.first['recipe_id'] as num?)?.toInt();
    if (topRecipeId != null) {
      final topRecipe = await db.getRecipe(topRecipeId);
      mostCookedRecipe = topRecipe?.name ?? '';
    }
  }
